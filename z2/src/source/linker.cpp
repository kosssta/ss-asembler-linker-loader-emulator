...
void Linker::link(list<string> input_files)
{
    ...
    for (string input_file : input_files)
    {
        ...
        unordered_map<string, unsigned> offset;
        ...
        while (num_symbols-- > 0)
        {
            ...
            unsigned id = symbols.addSymbol(name, secId == sectionId.end() ? symb.value : symb.value + offset[sections.findSection(secId->second)->name], secId == sectionId.end() ? nullptr : sections.findSection(secId->second), symb.defined, symb.global);
        ...
        for (RelocationTable::Record *r : relocations)
        {
            ...
            if(symb->section) 
                r->offset += offset[symb->section->name];
        }
        input.close();
    }
